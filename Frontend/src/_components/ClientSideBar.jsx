          ...
          userList &&  userList.map((item, key)=>{
            ...
            return (
                <div key={key}  className={`w-[180px] h-[150px] font-syn-regular grid grid-cols-2 gap-y-1 p-4 m-2 ${addClassName}`}>
                  ...
                  <span className="text-xs dark:text-dark-text"> hedge </span> <span className="text-xs dark:text-dark-text">   { (allInfo[item.id] && allInfo[item.id].hedgePip)?common.numberFormat(allInfo[item.id].hedgePip):0 }</span>
