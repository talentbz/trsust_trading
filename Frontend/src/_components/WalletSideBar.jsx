...
function WalletSideBar({show, setShow, dark}) {
  ...
  return (
    <div className={`${show ? "right-0" : "hidden"} w-[632px] h-[600px] bg-[#fff] border-[1.5px] rounded-l-3xl px-5 py-3 border-gray-300 z-30 transition-transform text-sm sidebar text-center dark:bg-[#454545] dark:border-[#6D6D6D]`}>
      <div className="flex items-center mt-5">
        <span onClick={() => { setShow(); setCurrentStep(1); }} className="float-left cursor-pointer ml-3">
      ...
      <div className="mt-[25px]">
        <p className="text-[14px] font-bold bg-gradient-to-r from-[#9327EB] to-30% to-[#3B93EB] text-transparent bg-clip-text mb-4 whitespace-nowrap font-syncopate-light">
          {currentStep === 3 ? "Status" : "Senden"}
        </p>
        {currentStep === 2 || currentStep === 3 || currentStep === 4 ? (
            <div className={`${currentStep === 3 ? "left-[251px] top-[84px]" : "left-[148px]" } absolute `}>
